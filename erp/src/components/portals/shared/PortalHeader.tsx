...
import { Building2, LogOut, Bell, User, KeyRound, Shield } from 'lucide-react';
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 px-3 sm:px-6 py-2.5 flex items-center justify-between gap-2 sm:gap-4 shadow-xs w-full max-w-full">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <button
            ...
            className="p-1.5 sm:px-2.5 sm:py-1.5 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 border border-slate-200 rounded-xl transition-all text-xs flex items-center gap-1.5 cursor-pointer"
            title="Change Account Password"
          >
            <KeyRound className="w-3.5 h-3.5 text-indigo-600" />
            <span className="hidden lg:inline font-semibold">Password Policy</span>
          </button>
          ...
          <button
            ...
            title="Click to manage password & account settings"
          >
            {currentUser.avatar ? (
              <img
